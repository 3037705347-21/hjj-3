...
import ScenarioTimelineShipBlock from './ScenarioTimelineShipBlock.vue';
...
const emit = defineEmits<{
  (e: 'edit-schedule', scheduleId: string): void;
  (e: 'conflicts-updated', conflicts: ScheduleConflict[]): void;
}>();
...
const startTime = computed(() => startOfHour(new Date()));
...
function getTimeFromX(x: number): Date {
  const effectiveX = x - berthLabelWidth;
  const hours = effectiveX / getPixelPerHour();
  return addHours(startTime.value, hours);
}

function getBerthFromY(y: number): string | null {
  const effectiveY = y - headerHeight;
  const idx = Math.floor(effectiveY / rowHeight);
  if (idx < 0 || idx >= sortedBerths.value.length) return null;
  return sortedBerths.value[idx].id;
}

function getBerthLabelWidthFn() {
...
<template>
  <div class="panel-border rounded-lg overflow-hidden">
    ...
    >
      ...
      <div
        class="absolute z-20 pointer-events-none border-r-2 border-harbor-red/60"
        :style="{
          left: `${currentTimeX}px`,
          top: '0px',
          bottom: '0px',
          display: currentTimeX > berthLabelWidth && currentTimeX < berthLabelWidth + totalHours * getPixelPerHour() ? 'block' : 'none',
        }"
      >
        <div class="absolute -left-[1px] -top-[1px] w-2 h-2 rounded-full bg-harbor-red border border-console-800" />
      </div>
      ...
      >
        ...
        >
          ...
          <rect
            ...
            :width="totalHours * getPixelPerHour()"
            :height="sortedBerths.length * rowHeight"
            fill="url(#grid-half-hours-scenario)"
          />
          <rect
            x="0"
            y="0"
