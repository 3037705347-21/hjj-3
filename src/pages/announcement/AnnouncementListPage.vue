...
import AnnouncementListTable from '../../components/announcement/AnnouncementListTable.vue';
import AnnouncementCreateModal from '../../components/announcement/AnnouncementCreateModal.vue';
import {
  Anchor,
  History,
  User,
...
const statsCards = computed(() => [
  {
    label: '已发布公告',
    value: announcementStore.publishedAnnouncements.length,
  ...
  {
    label: '置顶公告',
    value: announcementStore.pinnedAnnouncements.length,
  ...
  {
    label: '草稿箱',
    value: announcementStore.announcements.filter((a) => a.status === 'draft').length,
  ...
  {
    label: '已失效',
    value: announcementStore.expiredAnnouncements.length,
...
function handleCreated(announcementId: string) {
    ...
    <main class="p-4 space-y-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <h2 class="font-mono text-lg font-bold text-console-100">港区公告与值班提醒</h2>
          <span
            v-if="announcementStore.activeAnnouncements.length > 0"
            class="px-2 py-0.5 rounded-full bg-harbor-orange/20 text-harbor-orange text-xs font-mono font-medium border border-harbor-orange/40 animate-pulse"
          >
            {{ announcementStore.activeAnnouncements.length }} 条公告
          </span>
        </div>
        <button
      ...
      <AnnouncementListTable @view-detail="viewDetail" />
    </main>
